...
**Completed:** auth-service, user-service, transaction-service, ledger-service, report-service, notification-service, ai-service, sync-service, Alembic migrations (8 migrations), CI/CD (test + lint + build + deploy + coverage gate), Flutter mobile app (auth, dashboard, transactions, accounts, ledger, reports, settings, offline sync, org selection — 44+ Dart files), Kubernetes manifests (Kustomize base + staging/production overlays — 23 files), Next.js web dashboard (6 tabs + admin infra tab + org settings), Terraform IaC for GCP (6 modules — vpc, gke, cloudsql, memorystore, storage, iam), GCP staging live (all 8 services), multi-user organisations (full stack: Sprint 14 backend + Sprint 15 UI — OrgSwitcher, X-Org-ID header, OrgSelectionScreen, org scoping for all 8 services), org hardening (Sprint 16 — NOT NULL constraints, read_only enforcement, audit log, invite notifications), granular org permissions + transactional email delivery (Sprint 17 — accountant/staff/read_only custom roles via require_scope, SMTP welcome/invite emails — 180 tests)
**In Progress:** Sprint 18 planning — see `docs/SaaSpocalypse-Assessment.md` for agent-native foundations assessment and `docs/SPRINT-LOG.md` Sprint 17 "Deferred to Sprint 18" for candidate scope
**Not Started:** Sprint 18 — org deletion/transfer, TD-34 (LLM provider abstraction for ai-service), agent-native foundations (actor_type on audit_log, tool/contract layer, Public API + webhooks pulled forward from Phase 3)
