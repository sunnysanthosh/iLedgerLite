...
| 2026-06-15 | sprint-17-done | 1 | db-f1-micro | STOPPED | manual capture; started Cloud SQL to run local 4-gate test suite (180 unit + 13 smoke + 26 regression, all green), then stopped SQL again. **Note:** GKE cluster has only 1 node (vs ~3 for original always-on sizing) — all 9 app pods (8 services + Redis) in `ledgerlite-staging` namespace are `Pending` due to insufficient capacity. Staging is not currently serving traffic; separate from the billing issue. |
